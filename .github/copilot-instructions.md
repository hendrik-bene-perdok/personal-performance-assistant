...
- **Skills**: [.github/skills/](skills/) — each skill owns one capability (e.g. `goal`, `journal`, `review`, `prioritize`, `roadmap`, `meta-retro`). They are fully isolated and do not depend on each other. Templates are stored within the skill folders.
...
- **Write gate**: never modify a `workspace/` file without explicit user confirmation. See rule §3 and the internal workflow in `ppa.agent.md`.
