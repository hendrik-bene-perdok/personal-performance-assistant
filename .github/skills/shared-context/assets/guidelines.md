# LANGUAGE RULES
- **Default Language**: You MUST use [`LANGUAGE`] as the default language unless the user explicitly requests another language.
...
- [`GUIDELINES`]: "workspace/[guidelines_filename]"
- [`PROFILE`]: "workspace/[profile_filename]"
...
- **Missing Files**: If a file referenced by a variable does not exist, you MUST create it immediately using the corresponding template in `.github/skills/shared-context/templates/`.
...
- You MUST create the appropriate files in `workspace/`.
   - If a file does not exist in the `workspace/` folder, you MUST create it using the appropriate template from `.github/skills/shared-context/templates/`, translating both the filename and the content.
- You MUST use relative paths when referencing workspace files.

# TEMPLATE ADHERENCE RULES
- You MUST always check if there is a template available in `.github/skills/shared-context/templates/`.
...
- You MUST use the templates in `.github/skills/shared-context/templates/` for structure; fill only the placeholders with user data.
